...
async function main_member(page, i, id) {
  await page.goto(
    `https://www.airlines-manager.com/alliance/members/` + id
  );
  ...
  while (x < data.length / 6) {
    ...
    STAR = "STAR(S): " + (await sc.star_count_cl(data_HTML[x * 6 + 1]));
    HUB = "HUB(S): " + (await hb.list(data[x * 6 + 3]));
    VALORISATION = "VALORISATION: " + PI[0][1];
    SOLDE = "SOLDE: " + PI[0][3];
    LAST_CO = "LAST_CO: " + PI[0][2];
    ID = "ID: " + PI[1];
  ...
  await file.write("NUMBER OF MEMBERS: "+x);
  await file.write("\r");
}
