...
async function main_member(page) {
  ...
  for (i in id) {
    ...
    while (x < data.length / 6) {
      ...
      await file.write(COMPAGNY).then(
      await pi.value(page, data_HTML[x * 6 + 5]).then(
      await file.write(STAR).then(
      await file.write(HUB).then(
      await file.write("\n")))))
      x++;
    }
    console.log("--");
