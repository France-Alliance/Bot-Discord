...
async function dtfd(client) {
  ...
  fs.readFile(`../../bot_essentials/scrapper/data/${date.date()}-${date.month()}-${date.year()}.json`, 'utf8', (err, data) => {
    ...
    if (err) {
        ...
        for (i in al){
          a+=`Nom: ${al[i].Name}\rID: ${al[i].ID}\rClassement: ${al[i].Classement}\r${al[i].Profile}\r${al[i].Networks}\r\r`
          ...
          */
          //console.log(al[i])
          console.log(a)
          console.log(`Nom: ${al[i].Name}\rID: ${al[i].ID}\rClassement: ${al[i].Classement}\r${al[i].Profile}\r${al[i].Networks}\r\r`)
        }
