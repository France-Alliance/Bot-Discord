  ...
  if (command === "lastmsg") {
    ...
    if (userm.users.size === 0) {
      ...
      function time(msg){
        ...
        // seconds as 2 digits (ss)
        var seconds = ("0" + date_ob.getSeconds()).slice(-2);
        console.log("Date as YYYY-MM-DD hh:mm:ss ==> " + year + "-" + month + "-" + date + " " + hours + ":" + minutes + ":" + seconds);
         
        var dte = year + "-" + month + "-" + date + " " + hours + ":" + minutes + ":" + seconds;
        console.log("dte",dte)
        message.channel.send(dte)
    ...
    if (userm.users.size != 0) {
      ...
      function findID() {
        var user = userm.users.map((user) => user)
        console.log("user: ", user)
        var IDarray = userm.users.map((user) => user.lastMessageID)
        console.log("IDarray: ", IDarray)
        if (IDarray == null) {

          message.channel.send("no message since I rebooted")  
